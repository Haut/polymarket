...
let make_request (type a) ~(method_ : http_method) ~(path : string)
    ~(parser : string -> (a, error) result)
    ?(headers : (string * string) list = []) ?(params : params = []) ?body
    (t : t) : (a, error) result =
  let uri = build_uri t.base_url path params in
  let status, resp_body =
    match method_ with
    | GET -> do_get ~headers t uri
    | POST ->
        let body_str = Option.value ~default:"{}" body in
        do_post ~headers t uri ~body:body_str
    | DELETE -> do_delete ~headers t uri
  in
  handle_response status resp_body parser

let get (type a) ~(path : string) ~(parser : string -> (a, error) result)
    ?(headers : (string * string) list = []) ?(params : params = []) (t : t) :
    (a, error) result =
  make_request ~method_:GET ~path ~parser ~headers ~params t

let post (type a) ~(path : string) ~(parser : string -> (a, error) result)
    ?(headers : (string * string) list = []) ?(params : params = [])
    ~(body : string) (t : t) : (a, error) result =
  make_request ~method_:POST ~path ~parser ~headers ~params ~body t

let delete (type a) ~(path : string) ~(parser : string -> (a, error) result)
    ?(headers : (string * string) list = []) ?(params : params = []) (t : t) :
    (a, error) result =
  make_request ~method_:DELETE ~path ~parser ~headers ~params t

(** {1 Typed Parsing Helpers} *)

let json_parser f body = parse_json f body |> Result.map_error to_error

let json_list_parser f body =
  parse_json_list f body |> Result.map_error to_error

let text_parser body = Ok body
let unit_parser _body = Ok ()
