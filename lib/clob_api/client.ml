...
let create ?(base_url = default_base_url) ?credentials ?address ~sw ~net () =
  let http = Http_client.Client.create ~base_url ~sw ~net () in
...
let get_order_book t ~token_id () =
  [ ("token_id", [ token_id ]) ]
  |> Http_client.Client.get_json t.http "/book" order_book_summary_of_yojson
...
let get_order_books t ~token_ids () =
  ...
  in
  Http_client.Client.post_json_list t.http "/books" order_book_summary_of_yojson
    ~body []
...
let get_price t ~token_id ~side () =
  ...
  |> Http_client.Client.get_json t.http "/price" price_response_of_yojson

let get_midpoint t ~token_id () =
  [ ("token_id", [ token_id ]) ]
  |> Http_client.Client.get_json t.http "/midpoint" midpoint_response_of_yojson
...
let get_prices t ~requests () =
  let body =
    ...
    |> Yojson.Safe.to_string
  in
  Http_client.Client.post_json t.http "/prices" prices_response_of_yojson ~body
    []
...
let get_spreads t ~token_ids () =
  let body =
    ...
    |> Yojson.Safe.to_string
  in
  Http_client.Client.post_json t.http "/spreads" spreads_response_of_yojson
    ~body []
...
let create_order t ~order ~owner ~order_type () =
  ...
  Http_client.Client.post_json ~headers t.http path
    create_order_response_of_yojson ~body []
...
let create_orders t ~orders () =
  ...
  in
  let headers = get_auth_headers t ~method_:"POST" ~path ~body in
  Http_client.Client.post_json_list ~headers t.http path
    create_order_response_of_yojson ~body []

let get_order t ~order_id () =
  let path = "/data/order/" ^ order_id in
  let headers = get_auth_headers t ~method_:"GET" ~path ~body:"" in
  Http_client.Client.get_json ~headers t.http path open_order_of_yojson []

let get_orders t ?market ?asset_id () =
  let path = "/data/orders" in
  let headers = get_auth_headers t ~method_:"GET" ~path ~body:"" in
  []
  |> Http_client.Client.add "market" market
  |> Http_client.Client.add "asset_id" asset_id
  |> Http_client.Client.get_json_list ~headers t.http path open_order_of_yojson
...
let cancel_order t ~order_id () =
  ...
  [ ("orderID", [ order_id ]) ]
  |> Http_client.Client.delete_json ~headers t.http path
       cancel_response_of_yojson

let cancel_orders t ~order_ids () =
  let path = "/orders" in
  let headers = get_auth_headers t ~method_:"DELETE" ~path ~body:"" in
  let uri =
    Http_client.Client.build_uri (Http_client.Client.base_url t.http) path []
  in
  let status, resp_body =
    Http_client.Client.do_delete ~headers t.http
      (Uri.with_query uri [ ("orderIDs", order_ids) ])
  in
  Http_client.Client.handle_response status resp_body
    (fun body ->
      Http_client.Client.parse_json cancel_response_of_yojson body
      |> Result.map_error Http_client.Client.to_error)
    Http_client.Client.parse_error

let cancel_all t () =
  let path = "/cancel-all" in
  let headers = get_auth_headers t ~method_:"DELETE" ~path ~body:"" in
  Http_client.Client.delete_json ~headers t.http path cancel_response_of_yojson
    []
...
let get_trades t ?id ?taker ?maker ?market ?before ?after () =
  ...
  |> Http_client.Client.add "taker" taker
  |> Http_client.Client.add "maker" maker
  |> Http_client.Client.add "market" market
  |> Http_client.Client.add "before" before
  |> Http_client.Client.add "after" after
  |> Http_client.Client.get_json_list ~headers t.http path clob_trade_of_yojson
