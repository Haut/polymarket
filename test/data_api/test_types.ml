...
let test_side_to_string () =
  Test_utils.test_string_conversions ~to_string:string_of_side
    [ (BUY, "BUY"); (SELL, "SELL") ]

let test_side_roundtrip () =
  Test_utils.test_roundtrip ~to_json:yojson_of_side ~of_json:side_of_yojson
    ~equal:equal_side ~to_string:string_of_side [ BUY; SELL ]

(** {1 Activity Type Enum Tests} *)

let test_activity_type_to_string () =
  Test_utils.test_string_conversions ~to_string:string_of_activity_type
    [
      (TRADE, "TRADE");
      (SPLIT, "SPLIT");
      (MERGE, "MERGE");
...
let test_activity_type_roundtrip () =
  Test_utils.test_roundtrip ~to_json:yojson_of_activity_type
    ~of_json:activity_type_of_yojson ~equal:equal_activity_type
...
let test_trade_empty () =
  ...
  Alcotest.(check (option string))
    "proxy_wallet is None" None trade.proxy_wallet;
...
let test_activity_empty () =
  ...
  Alcotest.(check bool)
    "activity_type is None" true
    (Option.is_none activity.activity_type)
...
let test_holder_empty () =
  ...
  Alcotest.(check (option string))
    "proxy_wallet is None" None holder.proxy_wallet
...
let test_value_roundtrip () =
  let v =
    {
      user = Some "0x1234567890abcdef1234567890abcdef12345678";
      value = Some 5000.0;
    }
  in
...
let test_trader_leaderboard_entry_roundtrip () =
  let entry =
    {
      empty_trader_leaderboard_entry with
      rank = Some "1";
  ...
  Alcotest.(check bool)
    "roundtrip" true
    (equal_trader_leaderboard_entry entry result)
  ...
  [
    ...
    ( "other types",
      [
        ...
        ( "trader_leaderboard_entry roundtrip",
          `Quick,
          test_trader_leaderboard_entry_roundtrip );
      ] );
  ]
