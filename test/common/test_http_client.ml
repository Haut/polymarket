  ...
  Alcotest.testable
    (fun fmt params ->
      Format.fprintf fmt "[%s]"
        (String.concat "; "
           (List.map
              (fun (k, vs) -> Printf.sprintf "(%s, [%s])" k (String.concat ", " vs))
              params)))
    ( = )
...
let test_add_some () =
  ...
  Alcotest.(check params_testable) "adds key-value pair" [("key", ["value"])] result
...
let test_add_list_non_empty () =
  ...
  Alcotest.(check params_testable) "joins with comma" [("ids", ["1,2,3"])] result
...
let test_add_list_strings () =
  ...
  Alcotest.(check params_testable) "joins strings" [("tags", ["a,b,c"])] result
...
let test_add_list_with_commas () =
  ...
  let result = [] |> add_list "tags" Fun.id (Some ["a,b"; "c"]) in
  Alcotest.(check params_testable) "strings with commas" [("tags", ["a,b,c"])] result
...
let test_add_int_negative () =
  ...
  Alcotest.(check params_testable) "handles negative" [("offset", ["-10"])] result
...
let test_add_float_integer () =
  ...
  Alcotest.(check params_testable) "handles integer float" [("amount", ["100."])] result

let test_add_float_negative () =
  let result = [] |> add_float "price" (Some (-5.5)) in
  Alcotest.(check params_testable) "handles negative" [("price", ["-5.5"])] result

(** {1 URI Building Tests} *)

let test_build_uri_no_params () =
  let uri = build_uri "https://api.example.com" "/endpoint" [] in
  Alcotest.(check string) "builds basic URI" "https://api.example.com/endpoint" (Uri.to_string uri)

let test_build_uri_with_params () =
  let uri = build_uri "https://api.example.com" "/search" [("q", ["test"]); ("limit", ["10"])] in
  let uri_str = Uri.to_string uri in
  (* Verify the URI contains the expected parts *)
  Alcotest.(check bool) "starts with base" true
    (Test_utils.string_contains ~haystack:uri_str ~needle:"https://api.example.com/search");
  Alcotest.(check bool) "has query separator" true
    (Test_utils.string_contains ~haystack:uri_str ~needle:"?");
  Alcotest.(check bool) "contains q param" true
    (Test_utils.string_contains ~haystack:uri_str ~needle:"q=test");
  Alcotest.(check bool) "contains limit param" true
...
let test_build_uri_trailing_slash () =
  ...
  let uri = build_uri "https://api.example.com/" "/endpoint" [] in
  Alcotest.(check string) "handles trailing slash" "https://api.example.com//endpoint" (Uri.to_string uri)

let test_build_uri_empty_path () =
  let uri = build_uri "https://api.example.com" "" [] in
  Alcotest.(check string) "empty path" "https://api.example.com" (Uri.to_string uri)
...
let test_record_of_yojson json =
  ...
  { name = json |> member "name" |> to_string; value = json |> member "value" |> to_int }
...
let test_parse_json_list_empty () =
  ...
  let result = parse_json_list test_record_of_yojson json_str in
  match result with
  | Ok records -> Alcotest.(check int) "parses empty list" 0 (List.length records)
  | Error _ -> Alcotest.fail "expected success"
...
let test_parse_json_list_not_array () =
  ...
  match result with
  | Ok _ -> Alcotest.fail "expected error"
  | Error msg -> Alcotest.(check string) "expects array error" "Expected JSON array" msg
