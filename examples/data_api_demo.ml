(** Live demo of the Polymarket Data API client.
    ...
    This example calls all Data API endpoints and prints the results.
    Run with: dune exec examples/data_api_demo.exe

    Note: Some endpoints require valid user addresses or market IDs to return data.
    The demo uses sample values that may or may not have data.
*)
...
let sample_market = "0xdd22472e552920b8438f08c8830e189a5a159cc4e8d5f2fb0f0e8e9a7e3e2a5e"
...
let print_result_count name result =
  ...
  | Error err -> Printf.printf "[ERROR] %s: %s\n" name err.Common.Http_client.error
...
let run_demo env =
  ...
  (* Positions *)
  section "Positions";
  let positions = Data_api.Client.get_positions client
      ~user:sample_user
      ~limit:5
      () in
  ...
  (* Trades *)
  section "Trades";
  let trades = Data_api.Client.get_trades client
      ~limit:5
      () in
  print_result_count "get_trades (all)" trades;

  let user_trades = Data_api.Client.get_trades client
      ~user:sample_user
      ~limit:5
      () in
  ...
  (* Activity *)
  section "Activity";
  let activity = Data_api.Client.get_activity client
      ~user:sample_user
      ~limit:5
      () in
  ...
  (* Holders *)
  section "Holders";
  let holders = Data_api.Client.get_holders client
  ...
  (* Traded *)
  section "Traded";
  let traded = Data_api.Client.get_traded client
      ~user:sample_user
      () in
  (match traded with
   | Ok t ->
  ...
  (* Value *)
  section "Value";
  let value = Data_api.Client.get_value client
      ~user:sample_user
      () in
  ...
  let oi_market = Data_api.Client.get_open_interest client
      ~market:[sample_market]
      () in
  ...
  (* Live Volume *)
  section "Live Volume";
  let volume = Data_api.Client.get_live_volume client
      ~id:sample_event_id
      () in
  ...
  let closed = Data_api.Client.get_closed_positions client
      ~user:sample_user
      ~limit:5
      () in
  ...
  let builders = Data_api.Client.get_builder_leaderboard client
  ...
  let builder_vol = Data_api.Client.get_builder_volume client
      ~time_period:Data_api.Params.WEEK
      () in
  ...
  let traders = Data_api.Client.get_trader_leaderboard client
      ~category:Data_api.Params.OVERALL
      ~time_period:Data_api.Params.WEEK
  ...
  Printf.printf "Note: Empty results may indicate no matching data for sample parameters.\n"
